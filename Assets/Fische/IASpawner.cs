    ...
    void RandomiseGroups()
    {
        for (int i = 0; i < AIObject.Count(); i++)
        {
            if (AIObject[i].randomizeStats)
            {
                ...
                AIObject[i] = new AIObjects(AIObject[i].AIGroupName, AIObject[i].objectPrefab, Random.Range(1, 40), Random.Range(1, 20), Random.Range(1, 10), AIObject[i].randomizeStats);
